...
# Pre-processing
```{r Load count data}
seqdata <- read.delim(here("GSE193414_rawcounts.txt"), stringsAsFactors = FALSE) %>% # Import count data from this folder using here()
  drop_na() # Remove NAs
...
myCPM <- cpm(seqdata, normalized.lib.sizes=TRUE) # Obtain counts per million (CPM)
```
...
logcounts <- data.frame(log2(counts + 1)) # log2 counts
...
meta <- data.frame(id = colnames(seqdata)) %>% 
  ...
  mutate(day = gsub("P|Res", "", id),
         id = gsub("[0-9]","", id)) %>% 
  drop_na()
rownames(meta) <- colnames(seqdata[,2:19])
